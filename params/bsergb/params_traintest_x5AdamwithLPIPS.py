...
def traintest_BSERGB_x5AdamwithLPIPS(args):
    paths = ED()
    ...
    paths.save = ED()
    paths.save.save_path = './x5_interpolation'
    paths.save.exp_path = os.path.join(paths.save.save_path, f"{args.model_name}" + f"_x5adamwithLPIPS{args.extension}")
    ...
    training_config.crop_size = 384 if hostname == 'server' else 64
    ...
    training_config.interp_ratio = 20
    ...
    # training_config.optim.optim_params.weight_decay = 0.5
    ...
    training_config.optim.scheduler_params.T_max = 135000
    ...
    validation_config.val_epochs = 15
    validation_config.val_imsave_epochs = 15
    ...
    validation_config.interp_ratio = 20
