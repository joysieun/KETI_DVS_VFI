...
class Expv8_large(BaseModel):
    ...
    def net_validation(self, data_in, epoch):
        ...
        with torch.no_grad():
            ...
            data_recon = recon.cpu().numpy()
            data_gts = gts.cpu().numpy()
            recon = (data_recon - np.min(data_recon)) / (np.max(data_recon) - np.min(data_recon))
            gts = (data_gts - np.min(data_gts)) / (np.max(data_gts) - np.min(data_gts))

            recon = torch.Tensor(recon).cuda()
